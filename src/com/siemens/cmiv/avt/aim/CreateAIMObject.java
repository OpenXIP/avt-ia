...
public class CreateAIMObject {
	...
	public static void main(String[] args) {
		...
		aimCreator.marshallVolume(aim, seed, uid, dcm, "NA", "NA", "NA");
	}
	...
	public void marshallVolume(File aimFile, String seedPoint, String seedUID,
			File segDcm, String shape, String certainty, String comment) {
		try {
			...
			annot.setCodeValue("AVT004");
			annot.setCodeMeaning("Tumor Volume Annotation");
...
//			annot.setImagingObservationCollection(assmbleImageObservations());			
			...
			segFiles.add(segDcm);
			annot.setSegmentationCollection(assembleSegmentationCollection(segFiles, shape, certainty, comment));
	...
	public void marshallSeed(File aimFile, String seedPoint, String seedUID, File segDcm) {
		try {
			//create ImageAnnotation
			ImageAnnotation annot = factory.createImageAnnotation();
			annot.setId(BigInteger.ZERO);
			...
			annot.setCodingSchemeDesignator(aim_SchemeDesignator);
			annot.setCodingSchemeVersion(aim_SchemaVersion);
			annot.setCodeValue("AVT001");
			annot.setCodeMeaning("Tumor Seed Annotation");
...
//			annot.setImagingObservationCollection(assmbleImageObservations());			
			...
			//create imageReference collection
			annot.setImageReferenceCollection(assembleImageReferenceCollection(segDcm, seedUID));
	...
	public void marshallRECIST(File aimFile, File segDcm, String recist, String recistPoint,
		...
		try {
			...
			annot.setCodingSchemeDesignator(aim_SchemeDesignator);
			annot.setCodingSchemeVersion(aim_SchemaVersion);
			annot.setCodeValue("AVT002");
			annot.setCodeMeaning("Tumor RECIST Annotation");
	...
	public void marshallWHO(File aimFile, File segDcm, String who, String whoPoint, String whoUID) {
		try {
			...
			annot.setCodingSchemeDesignator(aim_SchemeDesignator);
			annot.setCodingSchemeVersion(aim_SchemaVersion);
			annot.setCodeValue("AVT003");
			annot.setCodeMeaning("Tumor WHO Annotation");
	...
	private CalculationCollection assmbleCalculationCollection(String value, String unit) {
		...
		if (unit.compareToIgnoreCase("mm") == 0){
			cal.setCodeMeaning("Long Axis of Tumor");
			cal.setCodeValue("AVT202");
...
//			cal.setMathML("");
		...
		if (unit.compareToIgnoreCase("mm2") == 0){
			cal.setCodeMeaning("Cross Product of Tumor Diameters");
			cal.setCodeValue("AVT203");
			cal.setCodingSchemeDesignator("AVT2");
//			cal.setMathML("");
			cal.setId(BigInteger.ZERO);
...
//		image.setSopClassUID(attrib.getSingleStringValueOrEmptyString());
	...
	private GeometricShapeCollection assmblePolylineShapeCollection( String seedPoint, String seedUID) {
		...
		for (int i = 0; i < buffer.length/4; i++){
			...
			String val = (buffer[4*i]).trim();
			twoDimSpaCoordinate.setX(Integer.parseInt(val));
			val = (buffer[4*i+1]).trim();
			...
			//point1
			twoDimSpaCoordinate = factory.createTwoDimensionSpatialCoordinate();
			twoDimSpaCoordinate.setId(BigInteger.valueOf(0));
			twoDimSpaCoordinate.setImageReferenceUID(seedUID);
			
			val = (buffer[4*i+2]).trim();
			twoDimSpaCoordinate.setX(Integer.parseInt(val));
			val = (buffer[4*i+3]).trim();
			twoDimSpaCoordinate.setY(Integer.parseInt(val));
			twoDimSpaCoordinate.setCoordinateIndex(BigInteger.valueOf(1));
			twoDimSpaCoordinate.setReferencedFrameNumber(BigInteger.valueOf(i));
	  ...
	  public AnatomicEntityCollection assmbleAnatomicEntity(){
	   ...
	   public SegmentationCollection assembleSegmentationCollection(List<File> segFiles, String shape, String certainty, String comment){
		   ...
		   for (int i = 0; i < segFiles.size(); i++){
				...
				observation.setCodeValue("RID3957");
				observation.setCodingSchemeDesignator("RadLex");
				if (!comment.isEmpty())
				...
				String radID = getShapeRadID(shape);
				if (!radID.isEmpty()){
					ImagingObservationCharacteristic observationCharacter0 = factory.createImagingObservationCharacteristic();
					observationCharacter0.setCodeMeaning(shape);
					observationCharacter0.setCodeValue(radID);
					observationCharacter0.setCodingSchemeDesignator("RadLex");
					observationCharacter0.setId(new BigInteger("0"));
					
					observationCharacterList.add(observationCharacter0);
				}
				
				radID = getCertaintyRadID(certainty);
				if (!radID.isEmpty()){
					ImagingObservationCharacteristic observationCharacter1 = factory.createImagingObservationCharacteristic();
					observationCharacter1.setCodeMeaning(certainty);
					observationCharacter1.setCodeValue(radID);
					observationCharacter1.setCodingSchemeDesignator("RadLex");
					observationCharacter1.setId(new BigInteger("0"));
	
					observationCharacterList.add(observationCharacter1);
				}
	...
	private String getCertaintyRadID(String certainty) {
		String radID = "";
		if (certainty.compareToIgnoreCase("almost certainly absent") == 0)
			radID = "RID38";
		
		if (certainty.compareToIgnoreCase("almost certainly present") == 0)
			radID = "RID32";
		
		if (certainty.compareToIgnoreCase("definitely not present") == 0)
			radID = "RID39";
		
		if (certainty.compareToIgnoreCase("definitely present") == 0)
			radID = "RID31";
		
		if (certainty.compareToIgnoreCase("possibly present") == 0)
			radID = "RID34";
		
		if (certainty.compareToIgnoreCase("probably not present") == 0)
			radID = "RID36";
		
		if (certainty.compareToIgnoreCase("probably present") == 0)
			radID = "RID33";
		
		return radID;
	}

	private String getShapeRadID(String shape) {
		String radID = "";
		if (shape.compareToIgnoreCase("asymmetrically shaped") == 0)
			radID = "RID5808";
		
		if (shape.compareToIgnoreCase("beaded") == 0)
			radID = "RID5813";
		
		if (shape.compareToIgnoreCase("curved") == 0)
			radID = "RID5816";
		
		if (shape.compareToIgnoreCase("globular") == 0)
			radID = "RID13428";
		
		if (shape.compareToIgnoreCase("irregularly shaped") == 0)
			radID = "RID5809";
		
		if (shape.compareToIgnoreCase("linear") == 0)
			radID = "RID5811";
		
		if (shape.compareToIgnoreCase("lobular") == 0)
			radID = "RID5801";
		
		if (shape.compareToIgnoreCase("nodular") == 0)
			radID = "RID5802";
		
		if (shape.compareToIgnoreCase("ovoid") == 0)
			radID = "RID5800";
		
		if (shape.compareToIgnoreCase("pedunculated") == 0)
			radID = "RID5806";
		
		if (shape.compareToIgnoreCase("plate-like") == 0)
			radID = "RID5810";
		
		if (shape.compareToIgnoreCase("polypoid") == 0)
			radID = "RID5805";
		
		if (shape.compareToIgnoreCase("round") == 0)
			radID = "RID5799";
		
		if (shape.compareToIgnoreCase("spoke-wheel") == 0)
			radID = "RID5814";
		
		if (shape.compareToIgnoreCase("straightened") == 0)
			radID = "RID5815";
		
		if (shape.compareToIgnoreCase("symmetrically") == 0)
			radID = "RID5807";
		
		if (shape.compareToIgnoreCase("wedge-shaped") == 0)
			radID = "RID5812";
