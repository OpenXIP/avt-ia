...
/**
 * @author Jie Zheng
 * Modified by Lawrence Tarbox to move Swing code onto the Swing Event Dispatch Thread.
 */
public class ImageAnnotation extends WG23Application implements WG23Listener, OutputAvailableListener{
	static ImageAnnotationFrame frame = null;			
	
	State appCurrentState;
	ApplicationDataManager dataMgr;
	

	public ImageAnnotation(URL hostURL, URL appURL) {
		super(hostURL, appURL);				
		final ImageAnnotation mainApp = this;
			...
			SwingUtilities.invokeAndWait(new Runnable() {
			    public void run() {
					try {
						...
						ImageAnnotationFrame frame = new ImageAnnotationFrame(mainApp);
	...
	public void setFrame(ImageAnnotationFrame frameIn)
	{
		frame = frameIn;
	}
	...
	public static void main(String[] args) {
		try {
			...
			new ImageAnnotation(hostURL, applicationURL);										
	...
	public String getSceneGraphInput(List<ObjectLocator> objLocs){
		...
		String filePath;				
		filePath = new File(objLocs.get(0).getUri()).getPath();
		String input = filePath.replace("file:\\", " ").trim();
		int index = input.lastIndexOf("\\");
		
		String folder = input.substring(0, index);
